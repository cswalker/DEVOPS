pipeline {
    ...
    parameters {
        ...
        choice(name: 'policy', choices: ['ROUND_ROBIN', 'LEAST_CONNECTIONS', 'IP_HASH'], description: 'Provide a policy for the new backend set')
        choice(name: 'health_check', choices: ['app', 'static'], description: 'Select a health_check for the new backend set')
    }
    stages {
        stage('Init') {
            steps {
                ...
                script {
                    ...
                    sh """
                        ansible-playbook -i inventory.yaml \
                            ...
                            -e arg_policy=${policy} \
                            -e arg_health_check=${health_check} \
                            provisioners/loadbalancer/ansible/backendset-update.yaml
                    """
                }
            }
        }
